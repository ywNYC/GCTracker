...
业主说「推」，commit `be23984` 推了 main。上线后立刻重测发现第一次读到的还是旧数据
（`batch.total=1`），是刚部署完的边缘缓存瞬时问题——3 秒后重试拿到了正确的 `total=20`，
跟这轮之前遇到的"推完立刻测容易撞见缓存"是同一类现象，不是真故障。

### 追加二：把"阶段"图换成"等待时长"直方图（业主原话「我一直想要排列的是……等待时长……
不是你这个，目前到什么程序」）

业主看完线上效果，指出一直想要的其实是"等待了多久"的分布（不到1年/1-2年/2-3年/3-5年/5年以
上这样分桶的 bar chart），不是"卡在哪个申请步骤"（递交/收件/指纹……）那张图——两者问的是完
全不同的问题，之前理解错了。

改法：
- `functions/api/tracker.js` 新增 `WAIT_BUCKETS`（5 档：<12/12-24/24-36/36-60/60+ 个月）+
  `waitHistogram(rows, myMonths)`，用跟 `popAgg`/`rank` 同一份合并后的大样本（D1 ∪ 订阅者）
  算每人的等待月数（`priority_date` 到 `d_approved` 或今天）分桶，返回 `{counts, max}`——跟
  已有的 `stageDist` 是同一个数据形状，可以共用同一个渲染组件。`batch`/`cat` 两个对象都带上
  `waitHist` 字段。
- `src/Tracker.jsx` 把渲染阶段直方图的组件从 `StageHistogram`（只认 `stageDist` 一种数据）
  改名泛化成 `BarHistogram`（吃 `{counts,max}` 这个通用形状），`CardView` 里新增"大家等了多
  久"板块用 `<BarHistogram dist={b.waitHist} />`，排在原来"大家走到哪一步了"（现在也用
  `BarHistogram` 渲染，改名后逻辑不变）**前面**——等待时长是主答案，阶段进度降级成补充信息，
  依然保留（不是所有场景都被业主否定，只是不该是首答）。`BatchView` 同样加了"大家等了多久"
  板块，位置在"各批次的人都堆在哪几季"和"大家走到哪一步了"之间。

等待时长直方图吃的是全量合并样本（不需要逐步日期），比只有 `stageN` 个人的阶段图样本大得
